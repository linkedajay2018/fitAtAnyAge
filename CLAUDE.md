...
pytest                           # or: pytest -v, pytest -k test_name
```
...
# Initialize a new language (one-time)
pybabel init -i messages.pot -d translations -l <code>

# Update existing languages after changes
...
After `pybabel update`, always check for `#, fuzzy` entries in `.po` files and fix them.
